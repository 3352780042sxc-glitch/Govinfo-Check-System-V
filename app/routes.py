...
from app.models import Info, User, Role, CrawlData
from app.forms import InfoForm, LoginForm, RegisterForm, CrawlerForm
from app.crawler import baidu_news_crawler, deep_crawl
...
def deep_crawl_api():
    ...
    if result:
        ...
        return jsonify({'success': False, 'error': '深度采集失败'})

@app.route('/save_crawl_data', methods=['POST'])
def save_crawl_data():
    ...
    for item in data:
        try:
            # 检查是否已存在该URL的数据
            existing_data = CrawlData.query.filter_by(original_url=item.get('original_url')).first()
            if existing_data:
                continue  # 跳过已存在的数据
            ...
            crawl_data = CrawlData(
                ...
                depth_crawled=item.get('depth_crawled', False)
            )
            
            # 如果有深度采集数据，保存深度采集信息
            if item.get('depth_data'):
                crawl_data.depth_title = item['depth_data'].get('title')
                crawl_data.depth_content = item['depth_data'].get('content')
                crawl_data.depth_images = ','.join(item['depth_data'].get('images', []))
                crawl_data.depth_crawled = True
